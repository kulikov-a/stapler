...
# defaults
freq=5                                 # <- debug! set to 5 min after debug
verb_l=2                               # <- dont talk too mach
...
validate="1"                           # verify OCSP signature by default. for debugging
always_update=""                       # dont fetch too often
cert_pem="/var/etc/cert.pem"           # <- debug. should be empty. read args
ocsp_bin="/var/etc/testocspstaple.der" # <- debug. should be empty. read args
issuer_pem="/var/etc/issuer.pem"       # <- debug. should be empty. read args
...
if [ -z "$always_update" ] && [ -L "$ocsp_bin" ] && [ -e "$ocsp_bin" ] && [ $freq -gt 0 ]; then
    ...
    if [ -n "$target_update" ] && [ $(($target_update + $min_freq_sec)) -lt $(($now + $run_freq_sec)) ]; then
        ...
        if [ $target_nextupdate -gt 0 ] && [ $(($now + $run_freq_sec)) -lt $target_nextupdate ]; then
            bye 0 2 "ocsp staple: ocsp response will be valid over next $freq minutes. skippig request this time" debug
        elif [ $target_nextupdate = 0 ] && [ $(($now + $run_freq_sec)) -lt $(($target_date + $min_freq2_sec)) ]; then
            bye 0 2 "ocsp staple: ocsp response have no nextUpdate field and updated less then $min_freq2 min. skippig request this time" debug
        else
...
resp_uri=$(openssl x509 -in "$cert_pem" -ocsp_uri -noout)
[ $? = 0 ] && [ -n "$resp_uri" ] || bye 1 1 "ocsp staple error: OCSP Responder URI not found" error
...
# fetch OCSP
resp=$(openssl ocsp -issuer "$issuer_pem" -cert "$cert_pem" -respout "$ocsp_tmp" -noverify -no_nonce -url "$resp_uri" ${ocsp_hh})
[ $? = 0 ] || bye 1 1 "ocsp staple error: ocsp request failed with return code $?. $resp" error
#echo resp: $resp
